...
class EditarColaboradorPage extends StatefulWidget {
  final Map<String, dynamic> colaborador;
  EditarColaboradorPage({required this.colaborador});

  @override
  _EditarColaboradorPageState createState() => _EditarColaboradorPageState();
}

class _EditarColaboradorPageState extends State<EditarColaboradorPage> {
  ...
  void initState() {
    super.initState();
    nombreController.text = widget.colaborador['nombre'] ?? '';
    apellidoPController.text = widget.colaborador['apellido_paterno'] ?? '';
    apellidoMController.text = widget.colaborador['apellido_materno'] ?? '';
    _cargarSucursal();
  }
  ...
  void _guardarColaborador() async {
    ...
    try {
      ...
      final res = await ApiService().editarColaborador(widget.colaborador['id'], data);
      if (mounted) {
        Navigator.pop(context, true);
      }
    } catch (e) {
      _mostrarError('Error al editar colaborador: ${e.toString()}');
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Editar Colaborador", style: TextStyle(color: Colors.white)),
